...
scene=c/'Resources/package/scenes/boot.json'
immersive=False
if scene.is_file():
    try: immersive=bool(json.loads(scene.read_text()).get('immersive_system_ui',False))
    except (OSError,ValueError,TypeError): pass
presentation='<key>LSUIPresentationMode</key><integer>4</integer>\n' if immersive else ''
plist='<?xml version="1.0" encoding="UTF-8"?>\n<plist version="1.0"><dict>\n'+''.join(tag(k,v) for k,v in vals.items())+'<key>CFBundlePackageType</key><string>APPL</string>\n<key>LSMinimumSystemVersion</key><string>'+min_os+'</string>\n<key>LSMinimumSystemVersionByArchitecture</key><dict><key>arm64</key><string>'+min_os+'</string></dict>\n<key>LSArchitecturePriority</key><array><string>arm64</string></array>\n<key>NSHighResolutionCapable</key><true/>\n'+presentation+'<key>NSPrincipalClass</key><string>NSApplication</string>\n</dict></plist>\n'
